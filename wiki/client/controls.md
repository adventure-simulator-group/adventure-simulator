...
| Hold <kbd>Shift</kbd> + <kbd>Left Alt</kbd> + any of <kbd>W</kbd><kbd>A</kbd><kbd>S</kbd><kbd>D</kbd> | RT + Left Stick Click | Dive. | The keyboard dive begins as soon as all three parts of the chord are held, regardless of the order in which they were pressed. It cannot repeat until at least one part is released. Movement input selects physical travel and the procedural lean; every direction uses the same authored `dive` upper-body pose. The chord consumes the armed prone toggle. `dive.glb` contributes only bones above the pelvis; when it is missing, `guard` supplies that layer. The runtime holds a directional 40-degree pelvis tilt through flight and composes the pelvis and legs from the load and eventual ground-contact poses. During recovery, the contact pose's directional yaw transfers progressively to the root so it preserves one world-space landing heading. The backward half-turn uses the opposite equivalent yaw branch from the downed pose, preventing the two 180-degree rotations from composing into a full flip. Dives launch lower and faster laterally than ordinary jumps. After authoritative terrain contact, a forward dive blends into prone and a backward dive into supine. A lateral dive instead blends directly into its matching side-roll midpoint, allowing held camera-following to continue without an intervening prone-idle pose; without held aim, that midpoint settles back to prone. |
