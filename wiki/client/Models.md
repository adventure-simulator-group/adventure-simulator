...
## Characters
For certain classes of objects that occur in nature, like [trees](https://en.wikipedia.org/wiki/File:Dragon_trees.jpg) or [terrain](https://commons.wikimedia.org/wiki/File:Terragen.jpg), procedural generation has been a solved problem for some time. For the remainder of this page, we will turn our focus to the procedural generation of humanoid characters, where the current state of the art is ~~the Nintendo Mii~~ [*Breath of the Wild* NPCs](https://www.reddit.com/r/Games/comments/kq4a65/npcs_in_the_legend_of_zelda_breath_of_the_wild/).

### Halbe's proposed algorithm
Below we propose a method of generating a character mesh, one which begins with a skeleton and iteratively adds more detail to achieve a lifelike appearance. By the fourth pass, we have a nude humanoid body with muscle and flesh, and the tools we've used to get us there can give us clothing and armor for free.

>Halbe: This is my magnum opus.
...
[^1]: All automated weighting techniques are mediocre, ours included. But in our approach, SDFs give us morph targets to refine the mediocrity.
...
By now, we have a mesher with an unambiguous, universal coordinate system for the surface of the body (from the [second pass](#second-pass-distance-fields-and-vertex-weights)) and a way to encode height (from the [third](#third-pass-heightmaps)). We can use this same mesher to produce meshes for body-conforming equipment entirely through texture data.
...
Unlike in the [third pass](#third-pass-heightmaps), we probably wouldn't have the heightmap for clothing with respect to the surface of the skin -- otherwise, a baggy shirt on a ripped guy would have abs -- but rather with respect to a "convex-only version" of the base body mesh. That is, we would generate a version of the base body's heightmap where any [convex surface](https://en.wikipedia.org/wiki/Convex_function) is pushed outwards until it is no longer convex.
