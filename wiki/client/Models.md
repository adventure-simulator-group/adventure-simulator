...
Let's say each bone is a [capsule](https://en.wikipedia.org/wiki/Capsule_(geometry)). Traverse through the [skeletal hierarchy](https://www.researchgate.net/figure/Human-skeleton-and-the-corresponding-hierarchical-structure_fig3_328011229) and generate a capsule mesh for each bone with the [vertices](https://en.wikipedia.org/wiki/Polygon_mesh#Vertices) all [skinned](http://wiki.polycount.com/wiki/Vertex_skinning) to that bone.

#### Second pass: distance fields and vertex weights
Next, we want to connect the capsule meshes. Connected bones' capsules are now [3D distance fields](https://en.wikipedia.org/wiki/Signed_distance_function) which combine with each other (this will smooth out the joints). There is a function to estimate how much influence a given point on the surface is getting from each bone, which determines its skinning weight.

We now have the basis for [constructive solid geometry](https://en.wikipedia.org/wiki/Constructive_solid_geometry), because the 3D distance field capsule primitives give us a function to place vertices onto. We will place vertices and build triangles according to a modified version of [advancing front](https://www.sciencedirect.com/topics/engineering/advancing-front-method):
