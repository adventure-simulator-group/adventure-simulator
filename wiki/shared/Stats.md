...
## Training
Skills increase on a much longer timescale than is conventional for RPGs, they are not increased via an abstract XP/leveling system and very little of their value comes from actually using them during gameplay. Instead they are *trained* during your character's off-screen daily schedule. The current interface allocates each of the 1,440 minutes in a day among explicit skill study, activities, and leisure. Activities currently include Prayer, Labor, Thievery, and Raiding; they train related skills at 25% speed while also producing morale, gold, notoriety, or encounter risk. Leisure is the remaining allocation and includes sleep; its bar shifts from green through yellow, orange, red, and black as time becomes insufficient.

Each skill and activity meter contains a draggable hourglass handle whose horizontal position represents its daily allocation; the rank fill behind a skill remains a separate display of the character's existing skill. Leisure is a derived bar. The UI displays hours in quarter-hour increments and only saves allocations in 15-minute steps, while the underlying schedule continues to store minutes. The browser performs reallocation locally before saving the completed schedule to the server, which applies both training and activity results over elapsed strategic time.
