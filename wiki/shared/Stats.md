...
## Training
Skills increase on a much longer timescale than is conventional for RPGs, they are not increased via an abstract XP/leveling system and very little of their value comes from actually using them during gameplay. Instead they are *trained* during your character's off-screen daily schedule. The current interface allocates each of the 1,440 minutes in a day among skill training, labor, and leisure. Leisure is the remaining allocation and includes sleep; its bar shifts from green through yellow, orange, red, and black as time becomes insufficient.

Each skill meter contains a draggable hourglass handle whose horizontal position represents its daily allocation; the rank fill behind it remains a separate display of the character's existing skill. Labor has the same draggable handle, while leisure is a derived bar. The UI displays hours in quarter-hour increments and only saves allocations in 15-minute steps, while the underlying schedule continues to store minutes. The browser performs all reallocation locally: increasing a skill or labor allocation takes minutes from leisure first, then labor, then proportionally from other skills. Decreasing an allocation returns minutes to leisure. The server only receives the completed schedule when it is saved, then applies its daily skill allocations over elapsed strategic time. Labor and leisure do not yet have gameplay effects.
