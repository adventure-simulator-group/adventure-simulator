...
The rank meter is a five-segment display using the same yellow-green, yellow, orange, red, and violet progression as equipment repair difficulty. Daily allocations are changed in 15-minute steps with the left/right buttons or mouse wheel. Clicking a displayed allocation opens an `hh:mm` text field; entered values snap to the nearest 15 minutes. The underlying schedule stores minutes, and the derived Leisure bar shows the unallocated remainder.
