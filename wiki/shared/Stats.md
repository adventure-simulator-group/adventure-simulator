...
## Training
Skills increase on a much longer timescale than is conventional for RPGs, they are not increased via an abstract XP/leveling system and very little of their value comes from actually using them during gameplay. Instead they are *trained* during your character's off-screen daily schedule. The current interface provides separate downtime and travel plans, each allocating its own 1,440 minutes among explicit skill study, activities, and leisure. Activities currently include Prayer, Labor, Thievery, and Raiding; they train related skills at 25% speed while also producing morale, gold, notoriety, or encounter risk where the character's context permits it. Leisure is the remaining allocation and includes sleep; its bar shifts from green through yellow, orange, red, and black as time becomes insufficient.

Each skill and activity meter contains separate downtime and travel handles whose horizontal positions represent the two daily allocations; the rank fill behind a skill remains a separate display of the character's existing skill. Each plan has its own derived Leisure bar. The UI displays hours in quarter-hour increments and only saves allocations in 15-minute steps, while the underlying schedule continues to store minutes. The browser reallocates each plan independently before saving both to the server, which selects the matching plan as strategic time advances.
