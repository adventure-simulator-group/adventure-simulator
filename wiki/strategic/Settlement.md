...
Settlement and quest destinations share the same strategic **location** shell. A settlement's base location view shows population statistics and historical alternative names on the left and a short population-based description on the right. Imported settlements may also expose a plain-text Viabundus settlement/city description in a collapsed section labeled with its source language. Its Map tab contains an accessible SVG map with roads, water, generalized GLO-30 height bands and contours, and explicitly partial Copernicus forest coverage in switchable atlas and paper styles. Cacheable world geometry is an external SVG layer; current and selected settlements, connectivity, and pin links stay in the dynamic inline overlay. Every canonical settlement pin can be inspected through the same `?destination=` URL used by the ordinary destination list. A directly connected settlement shows distance, journey time, and the existing travel action; any other settlement shows useful detail and an explicit no-direct-route message without a travel form. Drag, wheel, keyboard, and labeled controls progressively enhance pan and zoom, while links and server-rendered selection remain usable without JavaScript. Settlement services remain separate tabs and are available only at settlement locations.
