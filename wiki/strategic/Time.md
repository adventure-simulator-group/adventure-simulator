...
Implemented schedule effects include skill training, activities, and strategic-condition recovery. Every character saves one 24-hour settlement-downtime plan with integer-minute allocations for every skill plus Prayer, Labor, Thievery, and Raiding. Its unallocated Leisure remainder includes sleep. Walking advances personal time and travel condition but never trains skills or performs scheduled activities. Camp time first clears each member's fatigue; the remainder applies safe training, Prayer, and Leisure while masking Labor, Thievery, and Raiding. The pure training and settlement-activity calculations are shared with the native strategic simulation harness; the harness uses repeated one-day actions as its canonical cadence. Simulator profiles deliberately have no professed religion and require explicit per-tradition Religion allocations, so Prayer follows the same faithless Meditation contract in both the native and reducer-backed backends and never adds Religion study. A live bulk rest evaluates one aggregate outcome and at most one incident interruption, so rounded activity income and incidents can differ from an otherwise equivalent sequence of one-day rests; bulk-rest strategy parity remains follow-up work.
...
Religion study stores only direct hours in each of eight traditions. Correlated knowledge is derived from those direct hours and never fed back into storage. Auto-train is enabled by default: it spends the aggregate Religion budget on a character's professed religion, or splits it evenly in stable order across every religion represented by the current settlement's legal religious status. With Auto-train off, each tradition has its own explicit allocation. If neither a profession nor settlement target exists, automatic Religion time produces no training.

Combat study has the same optional Auto-train control and a separate aggregate budget. While enabled, current equipped hands determine the relevant Melee, Ranged, Dodge, and Block weights described in [Stats](../shared/Stats.md), and training deterministically catches the lowest normalized trained hours up before maintaining their weighted balance. While disabled, the four direct allocations are active instead. Each mode counts exactly one branch toward the daily plan, and changing equipment redirects future automatic training without rewriting the saved schedule.
...
- **Raiding** earns gold during downtime and feeds the same equipment-derived adaptive Combat distribution at 25% speed, independently of whether Combat Auto-train is enabled. It no longer prefers Ranged over Melee or derives Block and Dodge practice from armor. Raiding produces high notoriety and a high retaliation chance.
