...
shadow maps and bloom are disabled by default, while four-sample MSAA,
atmosphere IBL, material occlusion, baked tree-card depth, terrain normals, and
direct celestial lighting retain the scene's primary depth cues. Effects
unsupported by WebGPU, including Bevy 0.19 SSAO, are excluded rather than
retained as native-only paths. Review viewers can still enable compatible
...
scattered singles;
  ...
  preset supplies bounded tan/brown blade and vein palettes. These plates use
  the production oak morphology, front/back albedo, normal and AO maps, and a
  dedicated dry PBR response with zero canopy AO, low transmission,
