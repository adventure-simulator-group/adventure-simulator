...
The single `adventuresim-tactical-client` WASM artifact has explicit Bevy states for strategic map, strategic scene, and tactical startup. Bevy requires plugins to be registered before `App::run`, so tactical core, netcode, UI, and player infrastructure is registered in every mode but state-gated and inert during strategic rendering. Strategic states contain no `AdventureSimulatorClient` (and therefore open no socket), tactical camera/light/UI/player/controller/combat entities, or running physics simulation. This latent registration adds code size and plugin resources to the shared artifact; release artifact size and browser memory/CPU remain performance gates for the shared-binary experiment.

On a fight-capable quest page, the strategic scene contains only party participants already disclosed by the HTML response. The ordinary POST form remains canonical. After successful renderer startup, JavaScript may request JSON allocation without replacing the document, poll a session- and party-authorized handoff endpoint with capped backoff, and submit a versioned server/player command to the Bevy mailbox. The renderer exits the strategic state, despawns presentation entities, and uses the same tactical `OnEnter` setup as standalone `tactical.html`. A connection failure before the server supplies the controlled player restores the strategic scene and re-enables retry. After connection, the browser continues low-frequency authorized status monitoring; an ended mission navigates through its canonical mission-status URL so the strategic server performs the authoritative loot/location redirect. Polling is canceled on page hide, navigation, or renderer teardown and has no overall allocation deadline. Tactical exit removes the client, scene terrain, Replicon's actual `Replicated` roots, and their derived visual/collider descendants without touching strategic entities. Cylinder positions and idle transforms are presentation-only transient state and must never be persisted to SpacetimeDB.

Map meshes emit opaque marker IDs only. JavaScript resolves an exact `data-map-marker-id` against the server-rendered destination anchors, verifies that the canonical link is same-origin, and invokes that link. Contract `href` values are validation metadata and are never used by WASM to navigate. This preserves URL/SSR state, accessibility, and browser Back behavior.
