...
Fight-capable quest pages mount `strategic_scene` and retain the normal `/missions/enter` POST. Once WebGPU/WASM starts successfully, the loader intercepts that form with an explicit JSON `Accept`, polls `/api/missions/{mission_id}/handoff` with capped exponential backoff, and hands the ready connection to the same live Bevy app. It then monitors the authorized endpoint at low frequency until the server reports failure or completion; completion follows the canonical mission-status URL rather than synthesizing a client result. Page hide, navigation, and renderer teardown abort in-flight polling. Disable JavaScript/WebGPU, or follow the displayed mission-status link after an error, to exercise the ordinary redirect and standalone `tactical.html` fallback. Tactical plugins are registered but inert before handoff; use browser network/devtools to verify there is no tactical websocket in strategic mode. Map clicks are resolved through exact `data-map-marker-id` HTML links rather than contract URLs.

The first release build with latent tactical registration and live handoff produced a 153,056,239-byte wasm-bindgen module, up 384,610 bytes (about 0.25%) from the preceding shared-renderer commit. This is an artifact-size measurement only; browser memory, idle CPU, and transition latency still require profiling.
