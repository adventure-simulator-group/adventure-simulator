...
fn vista_pattern(position: vec3<f32>) -> f32 {
    let broad = sin(position.x * 0.018 + position.z * 0.013)
        * cos(position.z * 0.021 - position.x * 0.009);
    let detail = sin(position.x * 0.071 - position.z * 0.047)
        * cos(position.z * 0.063 + position.x * 0.039);
...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> FragmentOutput {
    ...
    let pattern = vista_pattern(position);
    ...
    let sward_color = color * vec3<f32>(0.80, 1.055, 0.70) * (0.965 + pattern * 0.045);
    color = mix(color, sward_color, sward * 0.52);

    // Reuse the production rock scan on exposed regional slopes. Triplanar
    ...
    let scanned_rock = rock_yz * weights.x + rock_xz * weights.y + rock_xy * weights.z;
    let rock_amount = slope * (1.0 - sward_coverage * 0.48);
    color = mix(color, scanned_rock * (0.86 + pattern * 0.08), rock_amount * 0.78);
    ...
    let snow_mask = vista_weather.y
        * smoothstep(0.28, 0.86, terrain_normal.y)
        * (0.94 + pattern * 0.06);
