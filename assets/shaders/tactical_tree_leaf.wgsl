...
struct TacticalTreeLeafMaterial {
    parameters: vec4<f32>,
}

@group(#{MATERIAL_BIND_GROUP}) @binding(0)
var<uniform> tree_leaf: TacticalTreeLeafMaterial;
...
fn vertex(vertex: Vertex) -> VertexOutput {
    ...
    // UV.y runs from the petiole attachment to the blade tip. Two spatially
    // offset waves avoid synchronized card motion while keeping the petiole
    // fixed and preserving the authored camber and torsion.
    let bend = smoothstep(0.03, 1.0, vertex.uv.y);
    let wind_direction = normalize(tree_leaf.parameters.xy);
    ...
    let time = globals.time * tree_leaf.parameters.w;
    ...
    ) * tree_leaf.parameters.z * bend * bend;
...
fn fragment(in: VertexOutput, @builtin(front_facing) is_front: bool) -> @location(0) vec4<f32> {
    ...
    let back_light = select(
        vec3<f32>(1.0),
        vec3<f32>(1.32, 1.16, 0.72),
        !is_front,
    );
    // Custom materials bypass StandardMaterial's lighting attenuation. Match
    // the accepted PBR foliage value before tonemapping rather than emitting
    // the raw vertex albedo as scene radiance.
    let oak_tint = vec3<f32>(0.18, 0.30, 0.08);
    let color = in.color.rgb * oak_tint * wrapped_light * spatial_hue * interior * back_light;
