...
export function triangulatePolygon(input) {
  ...
  const minimumDistance = extent * 1e-9, minimumArea = extent * extent * 1e-10;
  ...
  while (changed && points.length > 3) {
    ...
    points = points.filter((point, index) => {
      ...
      if (area <= minimumArea && between) { changed = true; return false; }
      return true;
    });
  }
...
function makeBuilder(materialName, label) {
  ...
  function triangle(a, b, c) {
    ...
    for (const point of [a, b, c]) {
      positions.push(...point); normals.push(...normal); colors.push(...material.color);
    }
  }
  return { positions, normals, colors, triangle, material, label };
}
...
export function signedVolume(mesh) {
  let volume = 0;
  for (let index = 0; index < mesh.positions.length; index += 9) {
    const a = mesh.positions.slice(index, index + 3), b = mesh.positions.slice(index + 3, index + 6), c = mesh.positions.slice(index + 6, index + 9);
    volume += (a[0] * (b[1] * c[2] - b[2] * c[1]) - a[1] * (b[0] * c[2] - b[2] * c[0]) + a[2] * (b[0] * c[1] - b[1] * c[0])) / 6;
  }
  return volume;
}

function move(point, offset) { return [point[0] + offset[0], point[1] + offset[1], point[2] + offset[2]]; }

function componentRange(component) {
  if (["grip", "slabGrip", "blade", "sectionBlade", "diamondBlade", "spear", "fork", "partisan", "glaive", "bill", "sleeve"].includes(component.kind)) return [0, component.length ?? 0];
  ...
  if (component.kind === "pommel" || component.kind === "socket") return [Math.min(...component.profile.map((point) => point[0])), Math.max(...component.profile.map((point) => point[0]))];
...
const COMPONENT_KINDS = new Set(["blade", "sectionBlade", "diamondBlade", "axe", "spear", "guard", "knuckleBow", "ringGuard", "tube", "figureEight", "fork", "partisan", "glaive", "fanPommel", "facetedBeak", "bill", "box", "pick", "beak", "hammer", "socket", "pommel", "collar", "sleeve", "mace", "grip", "slabGrip"]);
...
const KIND_KEYS = Object.fromEntries(Object.entries({
  blade: "length width thickness curvature taper singleEdge tipWidth belly", sectionBlade: "length width thickness taper section", diamondBlade: "length width thickness taper", axe: "width height thickness beard curvature side rootWidth upperShoulder lowerShoulder flare toe heel beardDrop",
  spear: "length width thickness shoulder rootWidth bellyPosition acuteness", guard: "width height thickness sweep controlWidth", knuckleBow: "width length bar thickness side bulge samples radialSegments", ringGuard: "radius bar arcStart arcEnd samples radialSegments", tube: "points radius radialSegments",
  figureEight: "width height bar samples radialSegments", fork: "length width baseWidth thickness crotch tineWidth tineTaper shoulderBlend crotchRound", partisan: "length width lugWidth thickness lugDrop bellyPosition rootWidth lugSweep acuteness", glaive: "length width thickness curvature root edgeCurvature spineCurvature bellyPosition pointLength rootLength",
  fanPommel: "width height thickness", facetedBeak: "length root tip thickness direction set bendPosition tipThickness", bill: "length width hook thickness root rootLength bellyPosition hookDepth hookCurvature pointLength", box: "size fitShaftSide", pick: "length radius direction", beak: "length radius direction curvature thickness rootSection tipSection bendPosition droop",
  hammer: "length face neck thickness direction crown neckRatio faceFlare crownLength faceThickness", socket: "profile segments fitShaft wall", pommel: "profile segments", collar: "width radius segments", sleeve: "length radius topRadius segments fitShaft wall", mace: "length rootRadius shoulderRadius cuspRadius cuspHeight concavity crownLength flanges flangeThickness profileSamples segments waist flangeDepth",
  grip: "length radius topScale wraps segments", slabGrip: "length width thickness scaleThickness",
...
const REQUIRED_FIELDS = {
  blade: ["length", "width", "thickness"], sectionBlade: ["length", "width", "thickness"], diamondBlade: ["length", "width", "thickness"], axe: ["width", "height", "thickness"], spear: ["length", "width", "thickness"],
  guard: ["width", "height", "thickness"], knuckleBow: ["width", "length"], ringGuard: ["radius"], tube: ["points", "radius"], figureEight: ["width"], fork: ["length", "width", "baseWidth", "thickness"],
  partisan: ["length", "width", "lugWidth", "thickness"], glaive: ["length", "width", "thickness"], fanPommel: ["width", "height", "thickness"], facetedBeak: ["length", "root", "thickness"], bill: ["length", "width", "hook", "thickness"],
  box: ["size"], pick: ["length", "radius"], beak: ["length", "radius"], hammer: ["length", "face", "neck", "thickness"], socket: ["profile"], pommel: ["profile"], collar: ["width", "radius"], sleeve: ["length", "radius"],
  mace: ["length", "rootRadius", "shoulderRadius", "cuspRadius", "flanges", "flangeThickness"], grip: ["length", "radius"], slabGrip: ["length", "width", "thickness"],
};
...
function schemaErrors(input) {
  ...
  input.components.forEach((component, index) => {
    ...
    if (!component || typeof component !== "object" || Array.isArray(component)) { errors.push(`${prefix} must be an object`); return; }
...
export function resolveDefinition(input) {
  const definition = JSON.parse(JSON.stringify(input)), frames = new Map(), errors = [], ids = new Set();
  frames.set("weapon.root", [0, 0, 0]);
  if (definition.shaft) { frames.set("shaft.bottom", [0, 0, 0]); frames.set("shaft.top", [0, definition.shaft.length, 0]); }
  for (let index = 0; index < definition.components.length; index += 1) {
    const component = definition.components[index], id = component.id ?? component.label ?? `component-${index}`;
    component.id = id;
    ...
    if (definition.shaft && ["socket", "sleeve"].includes(component.kind) && component.mount?.startsWith("shaft-top") && component.fitShaft !== false) {
      const contactRadius = definition.shaft.radius * (definition.shaft.topScale ?? 0.92), wall = component.wall ?? 0.003, outer = contactRadius + wall;
      component.fitShaft = true; component.wall = wall; component._shaftContactRadius = contactRadius;
      if (component.kind === "socket") component.profile = component.profile.map(([y]) => [y, outer]);
      else { component.radius = outer; component.topRadius = outer; }
    }
    ...
    if (component.mount?.startsWith("shaft-top")) {
      ...
      if (!shaftTop) errors.push(`${id}: shaft-top mount requires a shaft`);
        ...
        const anchor = rotatePoint([0, anchorY, 0], component.rotation), contact = move(anchor, component.offset), expected = [shaftTop[0] + local[0], shaftTop[1] + local[1] - (component.insertion ?? 0), shaftTop[2] + local[2]];
        component._resolvedAttachment = { target: "shaft.top", distance: length(subtract(contact, expected)), overlap: component.insertion ?? 0, contact, expected };
      }
    }
    if (component.stretchBetween) {
      const from = frames.get(component.stretchBetween[0]), to = frames.get(component.stretchBetween[1]);
      if (!from || !to) errors.push(`${id}: missing stretch frame ${!from ? component.stretchBetween[0] : component.stretchBetween[1]}`);
        ...
        component.offset = [from[0] + local[0], from[1] + local[1], from[2] + local[2]]; component.length = Math.max(0.001, to[1] - from[1]);
        const start = move(rotatePoint([0, 0, 0], component.rotation), component.offset), end = move(rotatePoint([(component.bar ?? 0.012) * (component.side ?? 1), component.length, 0], component.rotation), component.offset);
        component._resolvedStretch = { from: component.stretchBetween[0], to: component.stretchBetween[1], start, end, fromTarget: from, toTarget: to };
      }
      ...
      if (!target) errors.push(`${id}: missing attachment frame ${component.attach.to}`);
        ...
        const at = component.attach.at ?? "base", localY = at === "center" ? (range[0] + range[1]) / 2 : at === "top" ? range[1] : range[0];
        const delta = component.attach.offset ?? [0, 0, 0], overlap = component.attach.overlap ?? 0;
        const localAnchor = rotatePoint([0, localY, 0], component.rotation), expected = [target[0] + delta[0], target[1] + delta[1] - overlap, target[2] + delta[2]];
        component.offset = subtract(expected, localAnchor);
        const contact = move(localAnchor, component.offset);
        component._resolvedAttachment = { target: component.attach.to, distance: length(subtract(contact, expected)), overlap, contact, expected };
      }
    }
    const origin = component.offset ?? [0, 0, 0], resolvedRange = componentRange(component);
    const base = move(rotatePoint([0, resolvedRange[0], 0], component.rotation), origin), top = move(rotatePoint([0, resolvedRange[1], 0], component.rotation), origin), center = move(rotatePoint([0, (resolvedRange[0] + resolvedRange[1]) / 2, 0], component.rotation), origin);
    frames.set(`${id}.origin`, origin); frames.set(`${id}.base`, base); frames.set(`${id}.bottom`, base); frames.set(`${id}.top`, top); frames.set(`${id}.center`, center);
  }
  definition._resolutionErrors = errors; definition._frames = Object.fromEntries(frames);
  return definition;
}
...
function inverseRotatePoint([x, y, z], rotation = [0, 0, 0]) {
  const [rx, ry, rz] = rotation.map((degrees) => -degrees * Math.PI / 180);
  let a = x * Math.cos(rz) - y * Math.sin(rz), b = x * Math.sin(rz) + y * Math.cos(rz); x = a; y = b;
  a = x * Math.cos(ry) + z * Math.sin(ry); b = -x * Math.sin(ry) + z * Math.cos(ry); x = a; z = b;
...
export function lathe(profile, segments = 14, material = "wood", offset = [0, 0, 0], label = "lathe") {
  const largestRadius = Math.max(...profile.map((point) => point[1]));
  segments = Math.max(segments, Math.ceil(Math.PI * 2 * largestRadius / 0.012));
  const builder = makeBuilder(material, label);
  for (let ring = 0; ring < profile.length - 1; ring += 1) {
    for (let segment = 0; segment < segments; segment += 1) {
      ...
      const a = move([Math.cos(a0) * r0, y0, Math.sin(a0) * r0], offset);
      const b = move([Math.cos(a1) * r0, y0, Math.sin(a1) * r0], offset);
      const c = move([Math.cos(a1) * r1, y1, Math.sin(a1) * r1], offset);
      const d = move([Math.cos(a0) * r1, y1, Math.sin(a0) * r1], offset);
  ...
  const cap = (profileIndex, reverse) => {
    ...
    for (let segment = 0; segment < segments; segment += 1) {
      ...
      const a = move([Math.cos(a0) * radius, y, Math.sin(a0) * radius], offset);
      const b = move([Math.cos(a1) * radius, y, Math.sin(a1) * radius], offset);
      reverse ? builder.triangle(center, a, b) : builder.triangle(center, b, a);
    }
  };
  cap(0, true); cap(profile.length - 1, false);
  return finish(builder);
}

export function box(size, material, offset, label = "box") {
  const [x, y, z] = size.map((value) => value / 2);
  return prism([[-x, -y], [x, -y], [x, y], [-x, y]], z * 2, material, offset, label);
}
...
export function sampleAdaptiveCurve(evaluate, { minimumSegments = 8, maxChord = Infinity, maxDeviation = Infinity, maxDepth = 8 } = {}) {
  ...
  const refine = (t0, p0, t1, p1, depth) => {
    const midpointT = (t0 + t1) / 2, midpoint = evaluate(midpointT);
    const chord = Math.hypot(p1[0] - p0[0], p1[1] - p0[1]);
    const deviation = Math.max(
      pointSegmentDistance(evaluate(t0 * 0.75 + t1 * 0.25), p0, p1),
      pointSegmentDistance(midpoint, p0, p1),
      pointSegmentDistance(evaluate(t0 * 0.25 + t1 * 0.75), p0, p1),
    );
    if (depth < maxDepth && (chord > maxChord || deviation > maxDeviation)) {
      refine(t0, p0, midpointT, midpoint, depth + 1); refine(midpointT, midpoint, t1, p1, depth + 1);
    } else samples.push(p1);
  };
  const segments = Math.max(1, Math.floor(minimumSegments));
  samples.push(evaluate(0));
  for (let index = 0; index < segments; index += 1) {
    const t0 = index / segments, t1 = (index + 1) / segments;
...
export function sampleCubicBezier([p0, p1, p2, p3], quality) {
  return sampleAdaptiveCurve((t) => { const u = 1 - t; return [u ** 3 * p0[0] + 3 * u * u * t * p1[0] + 3 * u * t * t * p2[0] + t ** 3 * p3[0], u ** 3 * p0[1] + 3 * u * u * t * p1[1] + 3 * u * t * t * p2[1] + t ** 3 * p3[1]]; }, quality);
}

function appendCurve(target, points) { target.push(...points.slice(target.length ? 1 : 0)); return target; }

export function coneX(lengthValue, radius, direction, material, offset, label = "spike") {
  const builder = makeBuilder(material, label);
  const segments = 12, baseX = offset[0], tipX = offset[0] + lengthValue * direction;
  for (let segment = 0; segment < segments; segment += 1) {
    const a0 = segment / segments * Math.PI * 2, a1 = (segment + 1) / segments * Math.PI * 2;
  ...
  for (let segment = 0; segment < segments; segment += 1) {
    const a0 = segment / segments * Math.PI * 2, a1 = (segment + 1) / segments * Math.PI * 2;
    const a = [baseX, offset[1] + Math.cos(a0) * radius, offset[2] + Math.sin(a0) * radius], b = [baseX, offset[1] + Math.cos(a1) * radius, offset[2] + Math.sin(a1) * radius];
...
export function curvedBeakOutline(parameters) {
  ...
  const rootSection = parameters.rootSection ?? radius * 1.5, tipSection = parameters.tipSection ?? radius * 0.12;
  const bendPosition = parameters.bendPosition ?? 0.55, droop = parameters.droop ?? curvature * 0.35;
  const edge = (sign) => (t) => {
    const x = direction * beakLength * t;
    const bendExponent = Math.log(0.5) / Math.log(Math.max(0.15, Math.min(0.85, bendPosition)));
    const bend = Math.sin(Math.PI * Math.pow(t, bendExponent));
    const y = curvature * bend + droop * t, half = (rootSection * (1 - t) + tipSection * t) / 2;
    return [x, y + sign * half];
  };
  const quality = { minimumSegments: 12, maxChord: beakLength / 22, maxDeviation: Math.max(curvature, rootSection) / 180 };
  const upper = sampleAdaptiveCurve(edge(1), quality), lower = sampleAdaptiveCurve(edge(-1), quality);
...
export function hammerPoll(parameters, offset = [0, 0, 0], label = "hammer poll") {
  ...
  const s = direction, neckRatio = Math.max(0.05, Math.min(0.88, parameters.neckRatio ?? (parameters.neckLength ? parameters.neckLength / pollLength : 0.72))), neckLength = pollLength * neckRatio, faceFlare = parameters.faceFlare ?? 0;
  const faceHeight = face * (1 + faceFlare), crownReach = Math.max(0, parameters.crownLength ?? pollLength * crown);
  return prism([[0, -neck / 2], [neckLength * s, -neck / 2], [pollLength * s, -faceHeight / 2], [(pollLength + crownReach) * s, 0], [pollLength * s, faceHeight / 2], [neckLength * s, neck / 2], [0, neck / 2]], parameters.faceThickness ?? thickness, parameters.material ?? "steel", offset, label);
}
...
export function curvedBladeOutline(parameters) {
  ...
  const quality = { minimumSegments: 16, maxChord: bladeLength / 28, maxDeviation: Math.max(width, Math.abs(curvature)) / 220 };
  const left = sampleAdaptiveCurve(edge(-1), quality), right = sampleAdaptiveCurve(edge(1), quality);
  return [...left, ...right.reverse()];
}

export function axeHead(parameters, offset = [0, 0, 0], label = "axe head") {
  return prism(axeHeadOutline(parameters), parameters.thickness, "steel", offset, label);
}

export function axeHeadOutline(parameters) {
  const { width, height, thickness, beard = 0.35, curvature = 0.12, side = 1 } = parameters;
  const s = side;
  const socket = parameters.rootWidth ?? Math.min(width * 0.18, 0.055), flare = parameters.flare ?? 0;
  const upperShoulder = parameters.upperShoulder ?? 0.38, lowerShoulder = parameters.lowerShoulder ?? 0.26;
  const toe = parameters.toe ?? 0, heel = parameters.heel ?? 0, beardDrop = parameters.beardDrop ?? beard * 0.45;
  const edge = sampleAdaptiveCurve((t) => {
    const y = height * (0.42 - t * 0.9);
    return [width * (0.82 + flare * (t - 0.5) + curvature * Math.sin(Math.PI * t)) * s, y];
  }, { minimumSegments: 8, maxChord: Math.max(width, height) / 18, maxDeviation: width / 220 });
  const points = [
    [-socket * s, height * upperShoulder], [width * (0.68 + flare * 0.12) * s, height * (0.48 + toe)],
    ...edge,
    [width * beard * s, -height * (0.34 + beardDrop + heel)], [-socket * s, -height * lowerShoulder],
  ];
  return s < 0 ? points.reverse() : points;
}

export function diamondBlade(parameters, offset = [0, 0, 0], label = "diamond blade") {
  const builder = makeBuilder(parameters.material ?? "steel", label), samples = 14;
  for (let index = 0; index < samples; index += 1) {
    const t0 = index / samples, t1 = (index + 1) / samples;
    const y0 = t0 * parameters.length, y1 = t1 * parameters.length;
    const w0 = parameters.width * 0.5 * (1 - t0 * (parameters.taper ?? 0.92)), w1 = parameters.width * 0.5 * (1 - t1 * (parameters.taper ?? 0.92));
    const d0 = parameters.thickness * 0.5 * (1 - t0 * 0.7), d1 = parameters.thickness * 0.5 * (1 - t1 * 0.7);
    const rings = [[[ -w0, y0, 0], [0, y0, d0], [w0, y0, 0], [0, y0, -d0]], [[-w1, y1, 0], [0, y1, d1], [w1, y1, 0], [0, y1, -d1]]];
    for (let side = 0; side < 4; side += 1) { const next = (side + 1) % 4; builder.triangle(move(rings[0][side], offset), move(rings[0][next], offset), move(rings[1][next], offset)); builder.triangle(move(rings[0][side], offset), move(rings[1][next], offset), move(rings[1][side], offset)); }
  }
  return finish(builder);
}
...
export function fanPommel(parameters, offset = [0, 0, 0], label = "fan pommel") {
  ...
  for (let index = 0; index < 12; index += 1) { const t = index / 12; points.push([-width * (0.18 + 0.32 * Math.sin(t * Math.PI / 2)), height * (0.04 + 0.36 * t)]); }
  for (let index = 0; index <= 24; index += 1) { const angle = Math.PI - index / 24 * Math.PI; points.push([Math.cos(angle) * width / 2, height * 0.4 + Math.sin(angle) * height * 0.6]); }
  for (let index = 11; index >= 0; index -= 1) { const t = index / 12; points.push([width * (0.18 + 0.32 * Math.sin(t * Math.PI / 2)), height * (0.04 + 0.36 * t)]); }
  points.push([width * 0.18, 0]);
  return prism(points, thickness, parameters.material ?? "steel", offset, label);
}

export function facetedBeak(parameters, offset = [0, 0, 0], label = "faceted beak") {
  const { length: beakLength, root, tip = root * 0.16, thickness, direction = -1, set = 0 } = parameters, s = direction;
  const bendPosition = parameters.bendPosition ?? 0.22, bend = Math.max(0.12, Math.min(0.7, bendPosition));
  return prism([[0, -root / 2], [beakLength * bend * s, set * bend - root * 0.42], [beakLength * s, set - tip / 2], [beakLength * s, set + tip / 2], [beakLength * bend * s, set * bend + root * 0.42], [0, root / 2]], parameters.tipThickness ?? thickness, parameters.material ?? "steel", offset, label);
}
...
export function billHeadCurveSpans(parameters) {
  ...
  const rootLength = parameters.rootLength ?? 0.06, belly = parameters.bellyPosition ?? 0.48, hookDepth = parameters.hookDepth ?? 0.19, hookCurve = parameters.hookCurvature ?? 0.22, pointLength = parameters.pointLength ?? 0.24;
  const rootLeft = [-root, -rootLength], apex = [0, headLength], shoulder = [width, headLength * 0.68];
  const hookUpper = [width + hook * 0.72, headLength * (0.68 + hookCurve * 0.55)], hookTip = [width + hook, headLength * (0.68 - hookDepth)];
  const hookInner = [width + hook * 0.46, headLength * (0.62 + hookCurve * 0.2)], rootRight = [root, -rootLength];
  const shoulderHandle = Math.min(width, hook) * 0.16, crownHandle = hook * 0.18, innerHandle = hook * 0.16;
  return [
    { name: "body-to-apex", points: [rootLeft, [-root * 0.92, headLength * (1 - belly) * 0.42], [-root * 0.52, headLength * 0.84], apex], end: "apex" },
    { name: "apex-to-shoulder", points: [apex, [width * 0.1, headLength * (0.97 - pointLength * 0.08)], [shoulder[0] - shoulderHandle, shoulder[1]], shoulder], start: "apex", end: "shoulder" },
    { name: "shoulder-to-hook-crown", points: [shoulder, [shoulder[0] + shoulderHandle, shoulder[1]], [hookUpper[0] - crownHandle, hookUpper[1]], hookUpper], start: "shoulder", end: "hookUpper" },
    { name: "hook-crown-to-tip", points: [hookUpper, [hookUpper[0] + crownHandle, hookUpper[1]], [hookTip[0], headLength * (0.62 - hookDepth * 0.25)], hookTip], start: "hookUpper", end: "hookTip" },
    { name: "tip-to-hook-inner", points: [hookTip, [hookTip[0], headLength * (0.56 - hookDepth)], [hookInner[0] + innerHandle, hookInner[1]], hookInner], start: "hookTip", end: "hookInner" },
    { name: "hook-inner-to-root", points: [hookInner, [hookInner[0] - innerHandle, hookInner[1]], [width * 0.62, headLength * belly * 0.28], rootRight], start: "hookInner" },
  ];
}

export function billHeadOutline(parameters) {
  const { length: headLength, width, hook } = parameters;
  const quality = { minimumSegments: 3, maxChord: headLength / 28, maxDeviation: Math.min(width, hook) / 90 };
  const points = [];
...
export function maceFlangeOutline(component) {
  const half = component.length / 2, rootRadius = component.rootRadius, shoulderRadius = component.shoulderRadius;
  const cuspRadius = component.cuspRadius, cuspY = -half + component.length * component.cuspHeight;
  const samples = component.profileSamples ?? 10, exponent = 1.03 + Math.max(0, Math.min(0.98, component.concavity ?? 0)) * 2.97;
  const quality = { minimumSegments: samples, maxChord: component.length / 22, maxDeviation: (cuspRadius - Math.min(rootRadius, shoulderRadius)) / 150 };
...
export function maceFlangeMesh(component, material = "steel", label = "mace flange") {
  const outer = maceFlangeOutline(component), builder = makeBuilder(material, label), halfThickness = component.flangeThickness / 2;
  const inner = outer.map(([, y]) => { const t = (y + component.length / 2) / component.length; return [(component.rootRadius + (component.shoulderRadius - component.rootRadius) * t) * 0.55, y]; });
  const point = ([radius, y], z) => [radius, y, z];
  for (let index = 0; index < outer.length - 1; index += 1) {
    const o0 = outer[index], o1 = outer[index + 1], i0 = inner[index], i1 = inner[index + 1];
    builder.triangle(point(o0, halfThickness), point(i0, halfThickness), point(i1, halfThickness)); builder.triangle(point(o0, halfThickness), point(i1, halfThickness), point(o1, halfThickness));
    builder.triangle(point(o0, -halfThickness), point(i1, -halfThickness), point(i0, -halfThickness)); builder.triangle(point(o0, -halfThickness), point(o1, -halfThickness), point(i1, -halfThickness));
    builder.triangle(point(o0, -halfThickness), point(o0, halfThickness), point(o1, halfThickness)); builder.triangle(point(o0, -halfThickness), point(o1, halfThickness), point(o1, -halfThickness));
    builder.triangle(point(i0, -halfThickness), point(i1, halfThickness), point(i0, halfThickness)); builder.triangle(point(i0, -halfThickness), point(i1, -halfThickness), point(i1, halfThickness));
  }
  for (const index of [0, outer.length - 1]) {
    const outerPoint = outer[index], innerPoint = inner[index], reverse = index === 0;
    const a = point(innerPoint, -halfThickness), b = point(outerPoint, -halfThickness), c = point(outerPoint, halfThickness), d = point(innerPoint, halfThickness);
...
export function spearHead(parameters, offset = [0, 0, 0], label = "spear head") {
  ...
  const rootWidth = parameters.rootWidth ?? width * 0.4, bellyPosition = parameters.bellyPosition ?? shoulder, acuteness = parameters.acuteness ?? 1;
  const halfWidths = [], samples = 8;
  for (let index = 0; index <= samples; index += 1) {
    const t = index / samples;
    const half = t <= bellyPosition
      ? rootWidth / 2 + (width - rootWidth) / 2 * Math.pow(t / bellyPosition, 0.8)
      : width / 2 * Math.pow((1 - t) / (1 - bellyPosition), acuteness);
    halfWidths.push([half, t * headLength]);
  }
  const points = halfWidths.map(([half, y]) => [-half, y]);
  points.push(...halfWidths.slice(0, -1).reverse().map(([half, y]) => [half, y]));
  return prism(points, thickness, "steel", offset, label);
}

export function guard(parameters, offset = [0, 0, 0], label = "guard") {
  const { width, height, thickness, sweep = 0 } = parameters;
  const points = sampleAdaptiveCurve((t) => { const x = width * (t - 0.5), normalized = t * 2 - 1; return [x, sweep * normalized * normalized * normalized]; }, { minimumSegments: 20, maxChord: width / 18, maxDeviation: Math.max(height / 80, Math.abs(sweep) / 120) });
  const rod = tubePath(points, height * 0.38, "steel", offset, label, 8);
  const block = box([height * 1.35, height * 1.05, thickness * 1.25], "darkSteel", offset, `${label} block`);
  return mergeMeshes([rod, block]);
}
...
export function knuckleBow(parameters, offset = [0, 0, 0], label = "knuckle bow") {
  ...
  const points = sampleAdaptiveCurve((t) => { const u = 1 - t; return [2 * u * t * width * s + t * t * bar * s, 2 * u * t * (bowLength * 0.48 + bulge) + t * t * bowLength]; }, { minimumSegments: samples, maxChord: Math.max(width, bowLength) / samples, maxDeviation: Math.min(bar, thickness) / 10 });
  const radius = Math.min(bar, thickness) / 2, swept = tubePath(points, radius, parameters.material ?? "steel", offset, label, parameters.radialSegments ?? 8);
  const anchors = [points[0], points.at(-1)].map((point) => lathe([[-radius, radius * 0.8], [0, radius * 1.35], [radius, radius * 0.8]], 10, "darkSteel", [offset[0] + point[0], offset[1] + point[1], offset[2]], `${label} anchor`));
...
export function tubeCenterlineErrors(points, radius, closed = false, label = "tube") {
  const errors = [], segmentCount = points.length - 1, cumulative = [0];
...
export function tubePath(points, radius, material = "steel", offset = [0, 0, 0], label = "tube", radialSegments = 8, closed = false, allowCrossing = false) {
  if (!allowCrossing) { const errors = tubeCenterlineErrors(points, radius, closed, label); if (errors.length) throw new Error(errors.join("; ")); }
  radialSegments = tubeRadialSegments(radius, radialSegments);
  const builder = makeBuilder(material, label), count = points.length;
  const ringPoint = (index, segment) => {
    const previous = points[index === 0 ? (closed ? count - 2 : 0) : index - 1], next = points[index === count - 1 ? (closed ? 1 : count - 1) : index + 1];
    const dx = next[0] - previous[0], dy = next[1] - previous[1], tangentLength = Math.hypot(dx, dy) || 1;
    const nx = -dy / tangentLength, ny = dx / tangentLength, angle = segment / radialSegments * Math.PI * 2;
    return move([points[index][0] + nx * Math.cos(angle) * radius, points[index][1] + ny * Math.cos(angle) * radius, Math.sin(angle) * radius], offset);
  };
  const span = closed ? count - 1 : count - 1;
  for (let index = 0; index < span; index += 1) for (let segment = 0; segment < radialSegments; segment += 1) {
    const nextSegment = (segment + 1) % radialSegments, a = ringPoint(index, segment), b = ringPoint(index, nextSegment), c = ringPoint(index + 1, nextSegment), d = ringPoint(index + 1, segment);
    builder.triangle(a, b, c); builder.triangle(a, c, d);
  }
  if (!closed) for (const [index, reverse] of [[0, true], [count - 1, false]]) {
    const center = move([points[index][0], points[index][1], 0], offset);
    for (let segment = 0; segment < radialSegments; segment += 1) { const a = ringPoint(index, segment), b = ringPoint(index, (segment + 1) % radialSegments); reverse ? builder.triangle(center, b, a) : builder.triangle(center, a, b); }
  }
  return finish(builder);
}
...
export function tubeRadialSegments(radius, requested = 0, { maxChord = 0.006, maxSagitta = 0.0003 } = {}) {
  ...
  return Math.max(12, requested, chordSegments, sagittaSegments);
}

export function ringGuard(parameters, offset = [0, 0, 0], label = "side ring") {
  const samples = parameters.samples ?? 24, start = parameters.arcStart ?? 0, end = parameters.arcEnd ?? Math.PI * 2;
  const points = sampleAdaptiveCurve((t) => { const angle = start + t * (end - start); return [Math.cos(angle) * parameters.radius, Math.sin(angle) * parameters.radius]; }, { minimumSegments: samples, maxChord: Math.max(0.004, parameters.radius * 0.24), maxDeviation: Math.max(0.00025, parameters.radius / 220) });
...
export function figureEightGuard(parameters, offset = [0, 0, 0], label = "figure-eight guard") {
  const samples = parameters.samples ?? 48, halfWidth = parameters.width / 2, height = parameters.height ?? parameters.width * 0.28;
  const points = sampleAdaptiveCurve((t) => { const angle = t * Math.PI * 2; return [halfWidth * Math.sin(angle), height * Math.sin(angle) * Math.cos(angle)]; }, { minimumSegments: samples, maxChord: parameters.width / 30, maxDeviation: Math.min(parameters.bar ?? 0.009, height) / 12 });
...
export function sectionBlade(parameters, offset = [0, 0, 0], label = "sectioned blade") {
  const builder = makeBuilder(parameters.material ?? "steel", label), samples = 18;
  ...
  const ring = (t) => {
    ...
    if (style === "fullered") return [[-halfWidth, 0], [-halfWidth * 0.72, halfDepth], [-halfWidth * 0.28, halfDepth * 0.32], [0, halfDepth * 0.22], [halfWidth * 0.28, halfDepth * 0.32], [halfWidth * 0.72, halfDepth], [halfWidth, 0], [0, -halfDepth * 0.8]];
    return [[-halfWidth, 0], [0, halfDepth], [halfWidth, 0], [0, -halfDepth]];
  };
  for (let index = 0; index < samples; index += 1) {
    const t0 = index / samples, t1 = (index + 1) / samples, r0 = ring(t0), r1 = ring(t1), count = r0.length;
    for (let side = 0; side < count; side += 1) { const next = (side + 1) % count; const a = move([r0[side][0], t0 * parameters.length, r0[side][1]], offset), b = move([r0[next][0], t0 * parameters.length, r0[next][1]], offset), c = move([r1[next][0], t1 * parameters.length, r1[next][1]], offset), d = move([r1[side][0], t1 * parameters.length, r1[side][1]], offset); builder.triangle(a, b, c); builder.triangle(a, c, d); }
  }
  for (const [t, reverse] of [[0, true], [1, false]]) {
    const values = ring(t), center = move([0, t * parameters.length, 0], offset);
    for (let side = 0; side < values.length; side += 1) { const next = (side + 1) % values.length, a = move([values[side][0], t * parameters.length, values[side][1]], offset), b = move([values[next][0], t * parameters.length, values[next][1]], offset); reverse ? builder.triangle(center, b, a) : builder.triangle(center, a, b); }
  }
  return finish(builder);
}

export function forgedFork(parameters, offset = [0, 0, 0], label = "forged fork") {
  const { length: tineLength, width, baseWidth, thickness, crotch = 0.34 } = parameters, half = width / 2, root = baseWidth / 2, tine = parameters.tineWidth ?? width * 0.18;
  const taper = parameters.tineTaper ?? 0.55, shoulder = parameters.shoulderBlend ?? 0.2, crotchRound = parameters.crotchRound ?? 0.05;
  return prism([[-root, 0], [-half, tineLength * shoulder], [-half, tineLength], [-half + tine * taper, tineLength * 0.94], [-tine * 0.45, tineLength * (crotch + crotchRound)], [0, tineLength * crotch], [tine * 0.45, tineLength * (crotch + crotchRound)], [half - tine * taper, tineLength * 0.94], [half, tineLength], [half, tineLength * shoulder], [root, 0]], thickness, "steel", offset, label);
}

export function partisanBlade(parameters, offset = [0, 0, 0], label = "partisan blade") {
  const { length: bladeLength, width, lugWidth, thickness, lugDrop = 0.08 } = parameters, lug = lugWidth / 2;
  const belly = parameters.bellyPosition ?? 0.32, rootWidth = parameters.rootWidth ?? width * 0.18, lugSweep = parameters.lugSweep ?? 0.055, acuteness = parameters.acuteness ?? 1;
  const shoulderY = bladeLength * Math.max(0.18, Math.min(0.48, belly)), pointShoulder = shoulderY + (bladeLength - shoulderY) * (1 - 1 / (1 + acuteness));
  return prism([[0, bladeLength], [-width * 0.34, pointShoulder], [-width / 2, shoulderY], [-width * 0.34, bladeLength * 0.12], [-lug, bladeLength * lugDrop], [-lug * 0.72, 0], [-rootWidth / 2, bladeLength * lugSweep], [rootWidth / 2, bladeLength * lugSweep], [lug * 0.72, 0], [lug, bladeLength * lugDrop], [width * 0.34, bladeLength * 0.12], [width / 2, shoulderY], [width * 0.34, pointShoulder]], thickness, "steel", offset, label);
}

export function glaiveSpineCurveSpans(parameters) {
  const { length: bladeLength, width, curvature = 0.1 } = parameters, apexX = curvature * 0.42;
  const spineCurve = parameters.spineCurvature ?? 0.2, pointLength = parameters.pointLength ?? 0.24;
  const apex = [apexX, bladeLength], spineTop = [apexX - width * 0.1, bladeLength * (1 - pointLength)], spineLower = [-width * 0.42, bladeLength * 0.18];
  const spineJoinHandle = [-width * 0.08, -bladeLength * pointLength * 0.12];
  return [
    { name: "apex-to-spine", points: [apex, [apexX - width * 0.012, bladeLength * 0.96], [spineTop[0] - spineJoinHandle[0], spineTop[1] - spineJoinHandle[1]], spineTop], end: "spineTop" },
    { name: "spine-to-root", points: [spineTop, [spineTop[0] + spineJoinHandle[0], spineTop[1] + spineJoinHandle[1]], [-width * (0.34 + spineCurve * 0.04), bladeLength * 0.31], spineLower], start: "spineTop" },
  ];
}

export function glaiveOutline(parameters) {
  const { length: bladeLength, width, curvature = 0.1, root = 0.035 } = parameters, apexX = curvature * 0.42;
  const rootLength = parameters.rootLength ?? 0.08, belly = parameters.bellyPosition ?? 0.42, edgeCurve = parameters.edgeCurvature ?? 0.24, spineCurve = parameters.spineCurvature ?? 0.2, pointLength = parameters.pointLength ?? 0.24;
  const points = [[-root, -rootLength], [root, -rootLength], [root * 1.18, bladeLength * 0.025], [width * 0.48, bladeLength * 0.12]];
  const edgeLimit = 1 - pointLength * 0.34, quality = { minimumSegments: 12, maxChord: bladeLength / 28, maxDeviation: width / 180 };
  appendCurve(points, sampleAdaptiveCurve((u) => { const t = u * edgeLimit; return [apexX + (1 - t) * width * (0.54 + edgeCurve * Math.sin(Math.PI * Math.min(1, t / Math.max(0.1, belly)))), bladeLength * (0.08 + (0.84 - pointLength * 0.34) * t)]; }, quality));
  const nearApex = points.at(-1), apex = [apexX, bladeLength], spineTop = [apexX - width * 0.1, bladeLength * (1 - pointLength)], spineLower = [-width * 0.42, bladeLength * 0.18], rootShoulder = [-root * 1.18, bladeLength * 0.025];
...
export function measureMesh(mesh, parts = []) {
  ...
  return { bounds, dimensions, radius: length(dimensions) / 2, triangles: mesh.positions.length / 9, vertices: mesh.positions.length / 3, volume: Math.abs(volume), partCount: parts.length };
}

function valueAtPath(object, path) { return path.split(".").reduce((current, key) => current?.[key], object); }
...
function localComponentFootprint(component) {
  ...
  if (["socket", "pommel"].includes(component.kind)) { const radius = Math.max(...component.profile.map((point) => point[1])); return [radius, radius]; }
  ...
  if (["tube", "ringGuard", "figureEight", "knuckleBow"].includes(component.kind)) { const radius = component.radius ?? component.bar ?? 0.006; return [radius, radius]; }
  if (["blade", "sectionBlade", "diamondBlade"].includes(component.kind)) return [component.width / 2, component.thickness / 2];
  if (component.kind === "fanPommel") return [component.width / 2, component.thickness / 2];
  return [0.005, 0.005];
}

function componentFootprint(component, asParent = false) {
  if (!component) return [0, 0];
  const [halfX, halfZ] = localComponentFootprint(component), range = componentRange(component), halfY = asParent ? Math.abs(range[1] - range[0]) / 2 : 0;
  const axes = [rotatePoint([1, 0, 0], component.rotation), rotatePoint([0, 1, 0], component.rotation), rotatePoint([0, 0, 1], component.rotation)], extents = [halfX, halfY, halfZ];
...
export function closedManifoldErrors(mesh, label = "component") {
  const precision = 1e7, edges = new Map();
  const key = (offset) => mesh.positions.slice(offset, offset + 3).map((value) => Math.round(value * precision)).join(",");
  for (let index = 0; index < mesh.positions.length; index += 9) {
    const vertices = [key(index), key(index + 3), key(index + 6)];
    for (const [from, to] of [[vertices[0], vertices[1]], [vertices[1], vertices[2]], [vertices[2], vertices[0]]]) {
      const edge = from < to ? `${from}|${to}` : `${to}|${from}`, direction = from < to ? 1 : -1;
      const state = edges.get(edge) ?? [0, 0]; state[direction > 0 ? 0 : 1] += 1; edges.set(edge, state);
    }
  }
...
function validateWeaponUnchecked(definition, controls = []) {
  const errors = schemaErrors(definition);
  if (errors.length) return { valid: false, errors, resolved: null, mesh: null };
  let resolved;
  try { resolved = resolveDefinition(definition); } catch (error) { return { valid: false, errors: [`definition resolution failed: ${error.message}`], resolved: null, mesh: null }; }
  errors.push(...resolved._resolutionErrors);
  const finitePositive = (value, label, allowZero = false) => { if (!Number.isFinite(value) || (allowZero ? value < 0 : value <= 0)) errors.push(`${label} must be ${allowZero ? "non-negative" : "positive"}`); };
  if (resolved.shaft) { finitePositive(resolved.shaft.length, "shaft.length"); finitePositive(resolved.shaft.radius, "shaft.radius"); }
  ...
  for (const component of resolved.components) {
    ...
    if (component.profile) {
      component.profile.forEach((point, index) => { if (!Number.isFinite(point[0])) errors.push(`${component.id}.profile[${index}] height is not finite`); finitePositive(point[1], `${component.id}.profile[${index}] radius`); });
    ...
    const rawOffset = raw?.offset ?? [0, 0, 0], attachOffset = raw?.attach?.offset ?? [0, 0, 0], childFootprint = componentFootprint(component);
    if (raw.mount?.startsWith("shaft-top") && resolved.shaft) {
      const shaftRadius = resolved.shaft.radius * (resolved.shaft.topScale ?? 0.92), contactOffset = component.fitShaftSide ? component.offset : rawOffset;
    ...
    if (raw.attach) {
      const [ownerId, frameName = "origin"] = raw.attach.to.split("."), shaftOwner = ownerId === "shaft", target = ownerId === "weapon" || shaftOwner ? null : resolvedById.get(ownerId);
      ...
      const targetFrame = resolved._frames[raw.attach.to], contact = component._resolvedAttachment?.contact;
    ...
    if (raw.mount === "component-end") {
      const target = resolved.components.find((candidate) => candidate.id === raw.anchor || candidate.label === raw.anchor), targetFootprint = componentFootprint(target, true);
    ...
    if (raw.stretchBetween && component._resolvedStretch) {
      for (const [endName, point, targetPoint, frame] of [["start", component._resolvedStretch.start, component._resolvedStretch.fromTarget, raw.stretchBetween[0]], ["end", component._resolvedStretch.end, component._resolvedStretch.toTarget, raw.stretchBetween[1]]]) {
        const ownerId = frame.split(".")[0], target = ownerId === "shaft" ? null : resolvedById.get(ownerId), parentFootprint = ownerId === "shaft" && resolved.shaft ? [resolved.shaft.radius, resolved.shaft.radius] : componentFootprint(target, true);
    ...
    if (["socket", "sleeve"].includes(component.kind) && (component.mount?.startsWith("shaft-top") || raw.attach?.to?.startsWith("shaft.")) && resolved.shaft) {
      const frameName = raw.attach?.to?.split(".")[1] ?? "top", scale = frameName === "top" ? resolved.shaft.topScale ?? 0.92 : frameName === "center" ? ((resolved.shaft.bottomScale ?? 1) + (resolved.shaft.topScale ?? 0.92)) / 2 : resolved.shaft.bottomScale ?? 1;
      const contactRadius = resolved.shaft.radius * scale, wall = component.wall ?? 0.003;
    ...
    if (component.kind === "mace") {
      finitePositive(component.rootRadius, `${component.id}.rootRadius`); finitePositive(component.shoulderRadius, `${component.id}.shoulderRadius`); finitePositive(component.cuspRadius, `${component.id}.cuspRadius`);
  ...
  try {
    ...
    for (const part of mesh.parts) {
      if (!part.positions.length || part.positions.length % 9 !== 0) { errors.push(`${part.label}: part is empty or has incomplete triangles`); continue; }
      ...
      for (let index = 0; index < part.positions.length; index += 9) {
        const geometric = normalize(cross(subtract(part.positions.slice(index + 3, index + 6), part.positions.slice(index, index + 3)), subtract(part.positions.slice(index + 6, index + 9), part.positions.slice(index, index + 3))));
        if (dot(geometric, part.normals.slice(index, index + 3)) < 0.99) { errors.push(`${part.label}: stored normal disagrees with triangle winding`); break; }
      }
    }
  ...
  } catch (error) { return { valid: false, errors: [...errors, `build failed: ${error.message}`], resolved, mesh: null }; }
}

export function validateWeapon(definition, controls = []) {
  try {
    return validateWeaponUnchecked(definition, controls);
  } catch (error) {
    return { valid: false, errors: [`validation failed unexpectedly: ${error?.message ?? String(error)}`], resolved: null, mesh: null };
  }
}

export function buildWeapon(input) {
  const definition = resolveDefinition(input);
  const parts = [];
  const shaft = definition.shaft;
  if (shaft) {
    parts.push(lathe([[0, shaft.radius * (shaft.bottomScale ?? 1)], [shaft.length, shaft.radius * (shaft.topScale ?? 0.92)]], shaft.segments ?? 16, shaft.material ?? "wood", [0, 0, 0], "shaft"));
  }
  const add = (mesh, component, offset) => parts.push(transformMesh(mesh, component.rotation, offset));
  for (const component of definition.components) {
    ...
    if (component.kind === "guard") add(guard(component, [0, 0, 0], component.label), component, offset);
    ...
    if (component.kind === "fanPommel") add(fanPommel(component, [0, 0, 0], component.label), component, offset);
    ...
    if (component.kind === "box" && component.label?.includes("branch junction")) add(lathe([[-component.size[2] / 2, component.size[0] * 0.55], [0, component.size[0] * 0.8], [component.size[2] / 2, component.size[0] * 0.55]], 10, component.material ?? "darkSteel", [0, 0, 0], component.label), { ...component, rotation: [90, 0, 0] }, offset);
    ...
    if (component.kind === "socket") add(lathe(component.profile, component.segments ?? 14, component.material ?? "steel", [0, 0, 0], component.label), component, offset);
    if (component.kind === "pommel") add(lathe(component.profile, component.segments ?? 14, component.material ?? "steel", [0, 0, 0], component.label), component, offset);
    if (component.kind === "collar") add(lathe([[-component.width / 2, component.radius * 0.88], [-component.width * 0.32, component.radius], [component.width * 0.32, component.radius], [component.width / 2, component.radius * 0.88]], component.segments ?? 16, component.material ?? "steel", [0, 0, 0], component.label), component, offset);
    if (component.kind === "sleeve") add(lathe([[0, component.radius], [component.length, component.topRadius ?? component.radius * 0.88]], component.segments ?? 16, component.material ?? "darkSteel", [0, 0, 0], component.label), component, offset);
    if (component.kind === "mace") {
      ...
      const coreProfile = [[-half, rootRadius], [half * 0.72, rootRadius * 0.9], [half, shoulderRadius]];
      ...
      for (let index = 0; index < flanges; index += 1) {
        ...
        const flange = maceFlangeMesh({ ...component, rootRadius, shoulderRadius, cuspRadius, flangeThickness: component.flangeThickness ?? component.flangeDepth ?? 0.0025 }, component.material ?? "steel", `${component.label} flange`);
        parts.push(transformMesh(flange, [0, -angle * 180 / Math.PI, 0], offset));
      }
    }
    if (component.kind === "grip") {
      add(lathe([[0, component.radius], [component.length, component.radius * (component.topScale ?? 1)]], 14, component.material ?? "leather", [0, 0, 0], component.label), component, offset);
      const turns = component.wraps ?? 0;
      for (let turn = 1; turn < turns; turn += 1) {
        const y = offset[1] + component.length * turn / turns;
        parts.push(lathe([[-0.002, component.radius * 1.018], [0.002, component.radius * 1.018]], 16, component.wrapMaterial ?? "darkSteel", [offset[0], y, offset[2]], "grip binding"));
      }
    }
  ...
  const mesh = mergeMeshes(parts); mesh.resolvedDefinition = definition; return mesh;
}
