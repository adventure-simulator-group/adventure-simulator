...
pub struct ReshapeDefinition {
    pub cache: Arc<RwLock<Option<ComputePipeline>>>,
}

impl PartialEq for ReshapeDefinition {
    fn eq(&self, _other: &Self) -> bool {
        true
    }
}

impl ReshapeDefinition {
...
impl Reshape {
    pub fn execute(
        ...
        match input {
            ...
            _ => return Err(anyhow::anyhow!("Reshape input must be a buffer")),
        }
        match output {
            GpuResource::Buffer(b) => parameters.insert("output", b.clone()),
            _ => return Err(anyhow::anyhow!("Reshape output must be a buffer")),
        }
