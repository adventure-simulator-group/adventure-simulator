...
fn unaffordable_symptomatic_treatment_falls_back_to_natural_rest() {
    assert_eq!(
        choose_medical_action(MedicalActionContext {
            condition_status: adventuresim_core::morale::IncapacitationStatus::Staggered,
            symptomatic: true,
            at_settlement: true,
            intervention_available: true,
            purse: 6,
            observable_quote: Some(5),
            natural_rest_venue: Some(DomainSettlementActionService::Inn),
            medicated_rest_venue: None,
        }),
...
fn nonsymptomatic_convalescence_does_not_buy_medication() {
    assert_eq!(
        choose_medical_action(MedicalActionContext {
            ...
            purse: 100,
            observable_quote: Some(5),
            natural_rest_venue: Some(DomainSettlementActionService::Temple),
            medicated_rest_venue: Some(DomainSettlementActionService::Temple),
        }),
        (
...
fn affordable_symptomatic_treatment_buys_then_rests() {
    assert_eq!(
        choose_medical_action(MedicalActionContext {
            ...
            observable_quote: Some(5),
            natural_rest_venue: Some(DomainSettlementActionService::Inn),
            medicated_rest_venue: Some(DomainSettlementActionService::Inn),
        }),
...
fn medical_policy_uses_only_authorized_public_chart_and_patient_inventory() {
    ...
    for boundary in [
        ...
        assert!(
            chart.contains(boundary),
            "missing chart boundary {boundary}"
        );
    }
    ...
    for public_trace in [
        ...
        assert!(
            recovery.contains(public_trace),
            "missing trace {public_trace}"
        );
    }
...
fn owned_supported_medicine_does_not_require_a_storefront_quote() {
    ...
    let offers = source
        ...
        .nth(1)
        .and_then(|tail| {
            tail.split("pub(super) fn observable_medical_reserve")
                .next()
        })
