...
use crate::{
    ...
    character_skills__view, character_stats__view,
...
fn validate_tactical_receipt(
    ...
    for contact in &receipt.equipment_contacts {
        ...
        let equip = ctx
            ...
            .character_equip()
            .character_id()
            .find(contact.character_id)
            .ok_or("Tactical equipment contact owner has no equipment state")?;
        let slot = equip
            .is_equiped(contact.inventory_item_id)
        ...
        let valid_role = match contact.role {
            TacticalEquipmentContactRole::AttackerWeapon => {
                matches!(slot, ItemSlot::LeftHolding | ItemSlot::RightHolding)
                    && definition.kind == ItemKind::Weapon
            }
            TacticalEquipmentContactRole::DefenderEquipment => match slot {
                ItemSlot::LeftHolding | ItemSlot::RightHolding => {
                    matches!(definition.kind, ItemKind::Weapon | ItemKind::Shield)
                }
                ItemSlot::LeftArm
                | ItemSlot::RightArm
                | ItemSlot::LeftLeg
                ...
                | ItemSlot::Head
                | ItemSlot::Chest
                | ItemSlot::Stomach => definition.kind == ItemKind::Armor,
                _ => false,
            },
        };
        if !valid_role {
