...
#[test]
fn combat_power_gate_calibrates_against_bounded_autoresolve_seeds() {
    use adventuresim_core::autoresolve::{
        BattleOpening, BattleVictor, autoresolve_combat_power, resolve_battle,
    };

    let enemy = autoresolve_enemy(100, "cultist", 1, 10_000).unwrap();
    let enemy_power = autoresolve_combat_power(&enemy);
    let safe_party = vec![
        autoresolve_enemy(1, "cultist", 3, 10_000).unwrap(),
        autoresolve_enemy(2, "cultist", 3, 10_000).unwrap(),
    ];
    let safe_power = safe_party.iter().map(autoresolve_combat_power).sum::<u64>();
    assert!(safe_power.saturating_mul(4) >= enemy_power.saturating_mul(5));
    assert!((0..64).all(|seed| {
        resolve_battle(
            safe_party.clone(),
            vec![enemy.clone()],
            seed,
    ...
    let unsafe_party = vec![autoresolve_enemy(3, "cultist", 1, 10_000).unwrap()];
    let hard_enemy = autoresolve_enemy(101, "cultist", 6, 10_000).unwrap();
    let unsafe_power = unsafe_party
        .iter()
        .map(autoresolve_combat_power)
        .sum::<u64>();
    let hard_power = autoresolve_combat_power(&hard_enemy);
    assert!(unsafe_power.saturating_mul(4) < hard_power.saturating_mul(5));
    assert!((0..64).any(|seed| {
        resolve_battle(
            unsafe_party.clone(),
            vec![hard_enemy.clone()],
            seed,
            BattleOpening::Normal,
        )
        .victor
