// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{
	self as __sdk,
	__lib,
	__sats,
	__ws,
};


#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct LocalProblemSymptom {
    pub problem_id: String,
    pub settlement_id: String,
    pub symptom: String,
    pub public_summary: String,
    pub active_from: u64,
    pub active_until: u64,
}


impl __sdk::InModule for LocalProblemSymptom {
    type Module = super::RemoteModule;
}


/// Column accessor struct for the table `LocalProblemSymptom`.
///
/// Provides typed access to columns for query building.
pub struct LocalProblemSymptomCols {
    pub problem_id: __sdk::__query_builder::Col<LocalProblemSymptom, String>,
    pub settlement_id: __sdk::__query_builder::Col<LocalProblemSymptom, String>,
    pub symptom: __sdk::__query_builder::Col<LocalProblemSymptom, String>,
    pub public_summary: __sdk::__query_builder::Col<LocalProblemSymptom, String>,
    pub active_from: __sdk::__query_builder::Col<LocalProblemSymptom, u64>,
    pub active_until: __sdk::__query_builder::Col<LocalProblemSymptom, u64>,
}

impl __sdk::__query_builder::HasCols for LocalProblemSymptom {
    type Cols = LocalProblemSymptomCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        LocalProblemSymptomCols {
            problem_id: __sdk::__query_builder::Col::new(table_name, "problem_id"),
            settlement_id: __sdk::__query_builder::Col::new(table_name, "settlement_id"),
            symptom: __sdk::__query_builder::Col::new(table_name, "symptom"),
            public_summary: __sdk::__query_builder::Col::new(table_name, "public_summary"),
            active_from: __sdk::__query_builder::Col::new(table_name, "active_from"),
            active_until: __sdk::__query_builder::Col::new(table_name, "active_until"),

        }
    }
}

/// Indexed column accessor struct for the table `LocalProblemSymptom`.
///
/// Provides typed access to indexed columns for query building.
pub struct LocalProblemSymptomIxCols {
    pub problem_id: __sdk::__query_builder::IxCol<LocalProblemSymptom, String>,
    pub settlement_id: __sdk::__query_builder::IxCol<LocalProblemSymptom, String>,
}

impl __sdk::__query_builder::HasIxCols for LocalProblemSymptom {
    type IxCols = LocalProblemSymptomIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        LocalProblemSymptomIxCols {
            problem_id: __sdk::__query_builder::IxCol::new(table_name, "problem_id"),
            settlement_id: __sdk::__query_builder::IxCol::new(table_name, "settlement_id"),

        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for LocalProblemSymptom {}

