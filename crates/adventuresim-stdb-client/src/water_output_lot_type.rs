// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct WaterOutputLot {
    pub id: u64,
    pub container_object_id: u64,
    pub source_material_lot_id: u64,
    pub amount_ml: u64,
    pub contaminant_load_microunits: u64,
    pub concentration_anchor: f32,
    pub growth_per_hour: f32,
    pub anchor_minute: u64,
}

impl __sdk::InModule for WaterOutputLot {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `WaterOutputLot`.
///
/// Provides typed access to columns for query building.
pub struct WaterOutputLotCols {
    pub id: __sdk::__query_builder::Col<WaterOutputLot, u64>,
    pub container_object_id: __sdk::__query_builder::Col<WaterOutputLot, u64>,
    pub source_material_lot_id: __sdk::__query_builder::Col<WaterOutputLot, u64>,
    pub amount_ml: __sdk::__query_builder::Col<WaterOutputLot, u64>,
    pub contaminant_load_microunits: __sdk::__query_builder::Col<WaterOutputLot, u64>,
    pub concentration_anchor: __sdk::__query_builder::Col<WaterOutputLot, f32>,
    pub growth_per_hour: __sdk::__query_builder::Col<WaterOutputLot, f32>,
    pub anchor_minute: __sdk::__query_builder::Col<WaterOutputLot, u64>,
}

impl __sdk::__query_builder::HasCols for WaterOutputLot {
    type Cols = WaterOutputLotCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        WaterOutputLotCols {
            id: __sdk::__query_builder::Col::new(table_name, "id"),
            container_object_id: __sdk::__query_builder::Col::new(
                table_name,
                "container_object_id",
            ),
            source_material_lot_id: __sdk::__query_builder::Col::new(
                table_name,
                "source_material_lot_id",
            ),
            amount_ml: __sdk::__query_builder::Col::new(table_name, "amount_ml"),
            contaminant_load_microunits: __sdk::__query_builder::Col::new(
                table_name,
                "contaminant_load_microunits",
            ),
            concentration_anchor: __sdk::__query_builder::Col::new(
                table_name,
                "concentration_anchor",
            ),
            growth_per_hour: __sdk::__query_builder::Col::new(table_name, "growth_per_hour"),
            anchor_minute: __sdk::__query_builder::Col::new(table_name, "anchor_minute"),
        }
    }
}

/// Indexed column accessor struct for the table `WaterOutputLot`.
///
/// Provides typed access to indexed columns for query building.
pub struct WaterOutputLotIxCols {
    pub container_object_id: __sdk::__query_builder::IxCol<WaterOutputLot, u64>,
    pub id: __sdk::__query_builder::IxCol<WaterOutputLot, u64>,
}

impl __sdk::__query_builder::HasIxCols for WaterOutputLot {
    type IxCols = WaterOutputLotIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        WaterOutputLotIxCols {
            container_object_id: __sdk::__query_builder::IxCol::new(
                table_name,
                "container_object_id",
            ),
            id: __sdk::__query_builder::IxCol::new(table_name, "id"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for WaterOutputLot {}
