// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct LocalProblemIncidentReceipt {
    pub id: String,
    pub character_id: u64,
    pub problem_id: String,
    pub incident_id: String,
    pub learned_at: u64,
}

impl __sdk::InModule for LocalProblemIncidentReceipt {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `LocalProblemIncidentReceipt`.
///
/// Provides typed access to columns for query building.
pub struct LocalProblemIncidentReceiptCols {
    pub id: __sdk::__query_builder::Col<LocalProblemIncidentReceipt, String>,
    pub character_id: __sdk::__query_builder::Col<LocalProblemIncidentReceipt, u64>,
    pub problem_id: __sdk::__query_builder::Col<LocalProblemIncidentReceipt, String>,
    pub incident_id: __sdk::__query_builder::Col<LocalProblemIncidentReceipt, String>,
    pub learned_at: __sdk::__query_builder::Col<LocalProblemIncidentReceipt, u64>,
}

impl __sdk::__query_builder::HasCols for LocalProblemIncidentReceipt {
    type Cols = LocalProblemIncidentReceiptCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        LocalProblemIncidentReceiptCols {
            id: __sdk::__query_builder::Col::new(table_name, "id"),
            character_id: __sdk::__query_builder::Col::new(table_name, "character_id"),
            problem_id: __sdk::__query_builder::Col::new(table_name, "problem_id"),
            incident_id: __sdk::__query_builder::Col::new(table_name, "incident_id"),
            learned_at: __sdk::__query_builder::Col::new(table_name, "learned_at"),
        }
    }
}

/// Indexed column accessor struct for the table `LocalProblemIncidentReceipt`.
///
/// Provides typed access to indexed columns for query building.
pub struct LocalProblemIncidentReceiptIxCols {
    pub character_id: __sdk::__query_builder::IxCol<LocalProblemIncidentReceipt, u64>,
    pub id: __sdk::__query_builder::IxCol<LocalProblemIncidentReceipt, String>,
}

impl __sdk::__query_builder::HasIxCols for LocalProblemIncidentReceipt {
    type IxCols = LocalProblemIncidentReceiptIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        LocalProblemIncidentReceiptIxCols {
            character_id: __sdk::__query_builder::IxCol::new(table_name, "character_id"),
            id: __sdk::__query_builder::IxCol::new(table_name, "id"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for LocalProblemIncidentReceipt {}
