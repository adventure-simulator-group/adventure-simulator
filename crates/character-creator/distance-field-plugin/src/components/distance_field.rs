...
impl DistanceFieldComponent {
    ...
    pub fn update(
        mut fields: Query<(Entity, &mut DistanceFieldComponent, &GlobalTransform, Option<&Children>)>,
        shapes: Query<(&SdfShapeComponent, &GlobalTransform, Option<&SdfOperationComponent>)>,
    ) {
        for (_entity, mut distance_field, field_transform, children) in fields.iter_mut() {
            let relevant_shapes: Vec<(&SdfShapeComponent, &GlobalTransform, Option<&SdfOperationComponent>)> = if let Some(children) = children {
                children.iter()
