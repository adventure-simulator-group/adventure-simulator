...
impl ScatterDefinition {
    ...
    pub fn parse_signature(code: &str) -> Result<ScatterSignature> {
        ...
        for arg in &args {
            ...
            if parts.len() != 2 { return Err(anyhow::anyhow!("Invalid scatter parameter format: {}", arg)); }
            let name = parts[0];
            let ty_str = parts[1];
        ...
        if !has_value_param {
            return Err(anyhow::anyhow!("Scatter function must take a value parameter from the input domain."));
        }
    ...
    pub fn build_pipeline(
        ...
        transformed_code = re_sig.replace(&transformed_code, format!("fn scatter({})", new_args.join(", "))).to_string();
        ...
        if let Some(ref idx_ty) = sig.index_type {
            if idx_ty == "u32" {
                ...
                if input_res == ResourceType::Buffer { return Err(anyhow::anyhow!("vec2 index not supported for Buffer")); }
                full_code.push_str("    let index = _global_index.xy;\n");
            } else if idx_ty == "vec3<u32>" {
                if input_res != ResourceType::Texture3D { return Err(anyhow::anyhow!("vec3 index only supported for Texture3D")); }
