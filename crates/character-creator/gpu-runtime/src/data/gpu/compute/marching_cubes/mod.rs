...
fn gather(index: u32) -> u32 {{
    ...
    if (id0.x >= params.grid_x - 1u || id0.y >= params.grid_y - 1u || id0.z >= params.grid_z - 1u) {{
        return 0u;
    }}
...
fn interpolate(id1: vec3<u32>, id2: vec3<u32>, pos1: vec3<f32>, pos2: vec3<f32>) -> Vertex {{
    ...
    let t = (params.threshold - val1) / (val2 - val1);
    let n1 = calculate_normal(id1.x, id1.y, id1.z);
    let n2 = calculate_normal(id2.x, id2.y, id2.z);
    let pos = mix(pos1, pos2, t);
    let norm = normalize(mix(n1, n2, t));
...
fn stream(index: vec3<u32>, offset: u32, _res: ptr<storage, array<Vertex>, read_write>) {{
    if (index.x >= params.grid_x - 1u || index.y >= params.grid_y - 1u || index.z >= params.grid_z - 1u) {{
        return;
    }}
...
impl MarchingCubes {
    pub fn execute(
        ...
        sdf: &GpuResource,
        output_vertices: &GpuResource,
        output_indirect: &GpuResource,
        grid: (u32, u32, u32),
        threshold: f32,
        max_vertices: u32,
    ) -> Result<()> {
        ...
        if let GpuResource::Buffer(b) = output_indirect {
            sync_params.insert("indirect", b.clone());
        } else {
            return Err(anyhow::anyhow!("output_indirect must be a buffer"));
        }
        ...
        if let GpuResource::Buffer(out_v) = output_vertices {
            ...
            return Err(anyhow::anyhow!("output_vertices must be a buffer"));
        }
