...
impl<'a, T: bytemuck::NoUninit> TryFrom<(&'a WgpuContext, Resource<'a, T>)> for GpuResource {
    ...
    fn try_from(
        ...
        match resource_type {
            TestResourceType::Buffer => {
                Ok(Buffer::from_slice(context, resource, BufferDefinition::storage().with_copy_src())?.into())
            }
            TestResourceType::Texture2D(x, y, format) => {
                let texture_2d = Texture2D::new(context, Vec2::new(x as f32, y as f32), format)?;
...
where
    ...
    OUT: bytemuck::NoUninit + bytemuck::AnyBitPattern + PartialEq + std::fmt::Debug + Default + Copy,
    ...
    F: Fn(&WgpuContext, GpuResource, GpuResource) -> Result<()>,
{
    ...
    let input_resource_configs = all_resource_types(input_data.len(), &input_formats, in_element_components);
    let output_resource_configs = all_resource_types(expected_output.len(), &output_formats, out_element_components);
    ...
    for (in_config, in_tex_comps) in input_resource_configs {
        for (out_config, out_tex_comps) in &output_resource_configs {
            ...
            let inflated_output_init = inflate_flat(flat_expected, out_element_components, *out_tex_comps);

            let in_res: GpuResource = (&context, Resource(&inflated_input, in_config)).try_into()?;
            let out_res: GpuResource = (&context, Resource(&inflated_output_init, *out_config)).try_into()?;

            execute_op(&context, in_res, out_res.clone())?;

            let result_flat: Vec<S> = out_res.read(&context).await?;
            let result_deflated = deflate_flat(&result_flat, out_element_components, *out_tex_comps);
