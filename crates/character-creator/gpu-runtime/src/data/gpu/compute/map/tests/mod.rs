...
test_map!(
    ...
    [vec2(1.0f32, 2.0f32), vec2(2.0f32, 4.0f32), vec2(3.0f32, 6.0f32), vec2(4.0f32, 8.0f32)],
    f32
);
...
test_map!(
    ...
    [1.0f32, 2.0f32],
    [vec4(1.0f32, 2.0f32, 3.0f32, 4.0f32), vec4(2.0f32, 4.0f32, 6.0f32, 8.0f32)],
    f32
);
...
test_map!(
    ...
    [1i32, 2i32, 3i32, 4i32],
    [vec2(1i32, 2i32), vec2(2i32, 4i32), vec2(3i32, 6i32), vec2(4i32, 8i32)],
    i32
);
...
test_map!(
    ...
    [1u32, 2u32, 3u32, 4u32],
    [vec2(1u32, 2u32), vec2(2u32, 4u32), vec2(3u32, 6u32), vec2(4u32, 8u32)],
    u32
);
...
test_map!(
    ...
    [vec4(1.0f32, 2.0f32, 3.0f32, 4.0f32), vec4(5.0f32, 6.0f32, 7.0f32, 8.0f32)],
    [10.0f32, 26.0f32],
    f32
);
...
async fn test_generator() -> Result<()> {
    ...
    Map::execute(&context, &definition, None, &GpuResource::Buffer(output_buffer.clone()))?;
...
async fn test_extra_parameters() -> Result<()> {
    let context = WgpuContext::new().await.expect("Failed to init WGPU");
    let code = "fn map(val: f32, factor: f32, offset: f32) -> f32 { return val * factor + offset; }";
    let definition = MapDefinition::new(code.to_string())?;
