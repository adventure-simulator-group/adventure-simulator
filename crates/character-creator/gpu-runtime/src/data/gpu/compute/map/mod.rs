...
pub struct MapDefinition {
    pub code: String,
    pub cache: Arc<
        RwLock<HashMap<(Option<ResourceDescriptor>, ResourceDescriptor), Arc<(ComputePipeline, u64, u64)>>>,
    >,
}
        ...
        for arg in &args {
            ...
            if ty_str.contains("Resource") || ty_str.contains("ptr<") || ty_str.contains("texture_")
            {
                return Err(anyhow::anyhow!(
                    "Map function cannot take Resource types as parameters. Use Compute.Gather or Compute.Scatter instead."
                ));
            }
    ...
    pub fn build_pipeline(
        ...
        let (input_param_name, input_element_type, extra_params) = if let Some(_res) = &input_res {
            if sig.user_params.is_empty() {
                ...
                (Some(&sig.user_params[0].0), Some(sig.user_params[0].1.clone()), &sig.user_params[1..])
            }
        } else {
            (None, None, &sig.user_params[..])
        };
        ...
        if let Some(res) = &input_res {
            let ty_str = input_element_type.as_ref().unwrap_or(&ResourceBaseType::F32).as_str();
            let base_ty_str = input_element_type.as_ref().unwrap_or(&ResourceBaseType::F32).base_type().as_str();
        ...
        if let Some(ref size_ty) = sig.size_type {
            match size_ty.as_str() {
                ...
                "vec2<u32>" => match output_res {
                    ...
                    ResourceDescriptor::Texture2D(_) => full_code.push_str("    let size = tex_dim;\n"),
                    ResourceDescriptor::Texture3D(_) => {
                ...
                "vec3<u32>" => match output_res {
                    ...
                    ResourceDescriptor::Texture3D(_) => full_code.push_str("    let size = tex_dim;\n"),
                },
            ...
            match input_res {
                Some(ResourceDescriptor::Buffer(_)) => {
                    match output_res {
                        ...
                        ResourceDescriptor::Texture2D(_) => {
                            full_code.push_str("    let _in_global_index = global_id.y * tex_dim.x + global_id.x;\n");
                        }
                        ResourceDescriptor::Texture3D(_) => {
                            full_code.push_str("    let _in_global_index = (global_id.z * tex_dim.y + global_id.y) * tex_dim.x + global_id.x;\n");
                        }
                    }
                    full_code
                        .push_str("    if (_in_global_index >= arrayLength(&input)) { return; }\n");
                    full_code.push_str("    let in_val = input[_in_global_index];\n");
                }
                Some(ResourceDescriptor::Texture2D(_)) => {
                    full_code.push_str("    let _in_global_index = global_id.xy;\n");
                    full_code.push_str("    let in_tex_dim = textureDimensions(input);\n");
                    full_code.push_str("    if (_in_global_index.x >= in_tex_dim.x || _in_global_index.y >= in_tex_dim.y) { return; }\n");
                    let swizzle = match input_element_type.as_ref().unwrap_or(&ResourceBaseType::F32).component_count() {
                        1 => ".x",
                        2 => ".xy",
                        _ => "",
                    };
                    full_code.push_str(&format!(
                        "    let in_val = textureLoad(input, _in_global_index, 0){};\n",
                        swizzle
                    ));
                }
                Some(ResourceDescriptor::Texture3D(_)) => {
        ...
        match output_res {
            ...
            ResourceDescriptor::Texture2D(_) | ResourceDescriptor::Texture3D(_) => {
                ...
                let store_val = match sig_comps {
                    ...
                    2 => format!("vec4<{}>(_map_result, {}, {})", base_ty_str, pad_val, pad_val),
    ...
    pub fn execute_with_parameters(
        ...
        let input_descriptor = if let Some(input) = input {
            if sig.user_params.is_empty() {
                ...
                Some(ResourceDescriptor::from_resource(input, sig.user_params[0].1.clone()))
            }
        } else {
            None
        };
