...
async fn main() -> anyhow::Result<()> {
    ...
    let app = Router::new()
        ...
        .with_state(state)
        // Datastar overlay can be served from the same origin to avoid CORS/MIME issues.
        .nest_service(
            "/overlay",
...
async fn upsert_character_api(
    ...
    if let Err(err) = state.db.upsert_character(&character).await {
        eprintln!("failed to upsert character: {err:?}");
        return StatusCode::INTERNAL_SERVER_ERROR.into_response();
    }
...
async fn character_state_html(
    ...
    match render_overlay_inner_html(&state.db, &id, None).await {
        ...
        Err(RenderError::Db(err)) => {
            eprintln!("failed to render character state: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn get_character_json(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state.db.get_character(&id).await {
        ...
        Ok(None) => StatusCode::NOT_FOUND.into_response(),
        Err(err) => {
            eprintln!("failed to get character: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn get_inventory_json(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state.db.list_inventory(&id).await {
        ...
        Err(err) => {
            eprintln!("failed to list inventory: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn list_loot_bags(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state.db.list_loot_bags(&id).await {
        ...
        Err(err) => {
            eprintln!("failed to list loot bags: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn claim_loot_bag(
    ...
    match state.db.claim_loot_bag(&id, &req.character_id).await {
        ...
        Err(err) => {
            eprintln!("failed to claim loot bag: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn damage_character(
        ...
        .await
    {
        ...
        Err(err) => {
            eprintln!("failed to apply damage: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn respawn_character(State(state): State<AppState>, Path(id): Path<String>) -> impl IntoResponse {
    match state.db.respawn(&id).await {
        ...
        Err(err) => {
            eprintln!("failed to respawn: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn start_character_quest(
    ...
    if let Err(err) = state
        ...
        .await
    {
        eprintln!("failed to start character quest: {err:?}");
        return StatusCode::INTERNAL_SERVER_ERROR.into_response();
    }
...
async fn get_character_quest_status(
        ...
        .await
    {
        ...
        Err(err) => {
            eprintln!("failed to get character quest status: {err:?}");
            StatusCode::INTERNAL_SERVER_ERROR.into_response()
        }
    }
}
...
async fn complete_character_quest(
    ...
    if body.is_empty() {
        if let Err(err) = state
            ...
            .await
        {
            eprintln!("failed to complete character quest: {err:?}");
            return StatusCode::INTERNAL_SERVER_ERROR.into_response();
        }
    } else {
            ...
            .await
        {
            ...
            Err(err) => {
                eprintln!("failed to complete character quest with rewards: {err:?}");
                return StatusCode::INTERNAL_SERVER_ERROR.into_response();
            }
        }
    }
...
fn render_loot_html(loot_bags: &[LootBag]) -> String {
    ...
    for bag in loot_bags.iter().take(5) {
        ...
        let items = if bag.items.is_empty() {
            ...
            bag.items
                .iter()
                .map(|it| format!(r#"<code>{}</code>×{}"#, escape_html(&it.item_id), it.qty))
                .collect::<Vec<_>>()
                .join(" · ")
        };
