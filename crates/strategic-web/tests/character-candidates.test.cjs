...
function storage(initial = null, broken = false) { let value = initial; return { getItem() { if (broken) throw Error(); return value; }, setItem(_, next) { if (broken) throw Error(); value = next; }, value: () => value }; }
