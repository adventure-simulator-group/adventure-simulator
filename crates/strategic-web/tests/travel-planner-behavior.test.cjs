...
test("terrain rail preserves roads and woods around camps and reverses the return leg", () => {
  const helpers = plannerHelpers();
  const terrain = helpers.parseTerrain("road,0,30,2500,1000,0,0,0,0|open,30,20,2500,1000,0,0,0,0|sparse-woods,50,30,3000,800,200,0,0,0|deep-woods,80,20,3500,400,600,0,0,0");
  ...
  assert.deepEqual(Array.from(terrain, (span) => ({ ...span, weights: Array.from(span.weights) })), [
    { kind: "road", start: 0, duration: 30, check: 2.5, weights: [1000, 0, 0, 0, 0] },
    { kind: "open", start: 30, duration: 20, check: 2.5, weights: [1000, 0, 0, 0, 0] },
    { kind: "sparse-woods", start: 50, duration: 30, check: 3, weights: [800, 200, 0, 0, 0] },
    { kind: "deep-woods", start: 80, duration: 20, check: 3.5, weights: [400, 600, 0, 0, 0] },
  ]);
...
test("terrain parser rejects malformed, negative, zero-length, and discontinuous spans", () => {
  const helpers = plannerHelpers();
  assert.deepEqual(Array.from(helpers.parseTerrain("road,-1,5,0,1000,0,0,0,0")), []);
  assert.deepEqual(Array.from(helpers.parseTerrain("road,0,0,0,1000,0,0,0,0")), []);
  assert.deepEqual(Array.from(helpers.parseTerrain("road,0,5,0,1000,0,0,0,0|open,6,5,0,1000,0,0,0,0")), []);
  assert.deepEqual(Array.from(helpers.parseTerrain("road,0,5,extra")), []);
  assert.deepEqual(Array.from(helpers.parseTerrain("lava,0,5,0,1000,0,0,0,0")), []);
  assert.deepEqual(Array.from(helpers.parseTerrain("road,0,5,0,999,0,0,0,0")), []);
  assert.deepEqual(
    Array.from(helpers.parseTerrain("wetland,0,5,4000,0,0,0,1000,0"), (span) => Array.from(span.weights)),
    [[0, 0, 0, 1000, 0]],
  );
});
...
test("authoritative travel guards stale sync, bounded itineraries, and terminal provision use", () => {
  const strategic = readRustModuleSource(path.join(root, "..", "adventuresim-stdb-module", "src", "strategic", "mod.rs"));
  const time = readRustModuleSource(path.join(root, "..", "adventuresim-stdb-module", "src", "time", "mod.rs"));
...
test("alcohol chronology, authority, and automatic surgery consumption stay reducer-authoritative", () => {
  ...
  const time = readRustModuleSource(path.join(moduleRoot, "time", "mod.rs"));
