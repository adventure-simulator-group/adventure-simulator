...
async fn create_character(
    ...
    let id = format!("char-{}", chrono_id());

    let _ = state
        .db
        .call("create_character", &[json!(id.clone()), json!(form.name)])
        .await;
...
async fn show_character(
    ...
    let is_current = session.character_id() == Some(id.as_str());
...
async fn update_character(
    ...
    let _ = state
        .db
        .call("update_character", &[json!(id.clone()), json!(form.name)])
        .await;
