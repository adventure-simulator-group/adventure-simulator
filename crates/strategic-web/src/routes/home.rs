...
async fn home(State(state): State<AppState>, session: Session) -> Html<String> {
    ...
    let (current_settlement, active_party, active_quest) = if let Some(character) = current_character
    {
        // Get settlement
        let settlement: Option<Settlement> = if let Some(settlement_id) = &character.current_settlement_id {
            let settlements: Vec<Settlement> = state
                .db
                .query(&format!("SELECT * FROM settlement WHERE id = '{}'", settlement_id))
                .await
                .unwrap_or_default();
        ...
        let quest: Option<Quest> = if let Some(quest_id) = party.as_ref().and_then(|p| p.active_quest_id.as_ref()) {
