    ...
    let settlements = std::mem::take(&mut draft.settlements)
        ...
        .map(|(mut land, cover)| {
            ...
            push_source_note(
                &mut land,
                if fallback {
                    "**[Copernicus HRL Forests](https://land.copernicus.eu/en/products/high-resolution-layer-forests-and-tree-cover):** At least one TCD/DLT source value was missing or reserved; forest density/type uses the documented deterministic HYDE 3.5/elevation fallback."
                } else {
