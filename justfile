...
tactical mission_id=env_var_or_default("TACTICAL_MISSION_ID", "test-mission") scene_key=env_var_or_default("TACTICAL_SCENE_KEY", "hills") bots=env_var_or_default("TACTICAL_BOTS", "3") port=env_var_or_default("TACTICAL_PORT", tactical_port) url=env_var_or_default("TACTICAL_SPACETIMEDB_URL", spacetime_url) module=env_var_or_default("TACTICAL_SPACETIMEDB_MODULE", spacetime_module):
	@cargo run --package adventuresim-tactical-server --features "debug" -- \
		...
		--scene-key {{scene_key}} \
		--spacetimedb-url {{url}} \
		--spacetimedb-module {{module}} \
		--required-enemy-kills {{bots}} \
		--no-timeout
...
tactical-isolated profile="tactical-dev" base_port="23200" mission_id="test-mission" scene_key="hills" character_id="0" bots="3": preflight verify-db-client build-tactical
	@{{python_bin}} scripts/dev_stack.py run-profile --mode tactical {{quote(profile)}} {{quote(base_port)}} \
		--mission-id {{quote(mission_id)}} --scene-key {{quote(scene_key)}} \
		--character-id {{quote(character_id)}} --enemy-count {{quote(bots)}}
