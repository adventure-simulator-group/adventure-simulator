...
The map and small strategic scenes use the same production Bevy/WASM artifact as tactical combat. A versioned startup contract selects `strategic_map`, `strategic_scene`, or `tactical`. Tactical core, netcode, player, and UI plugins are registered up front because Bevy cannot add them after the app starts, but remain latent in strategic states: no client/socket, tactical camera, light, UI, player, controller, combat entity, or physics simulation exists until a validated handoff changes state. Server-rendered SVG/HTML remains the accessible fallback when WebGPU or WASM initialization is unavailable.
