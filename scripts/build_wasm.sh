...
# Bevy web asset loading can be sensitive to how the page is served.
# Mirror loose files into the static root as a fallback for root-relative requests.
find "$ASSET_DIR" -maxdepth 1 -type f -print0 | while IFS= read -r -d '' file; do
  cp "$file" "$STATIC_DIR/$(basename "$file")"
done
